...
const EditPlantForm = ({close, handleEdit, lightsources, plant}) => {
    ...
    const formik = useFormik({
        initialValues: {
          name: plant.name,
          water_date: moment(plant.water_schedule[0].water_date).format('yyyy-MM-DD'),
          file: { size: 0, type: 'image/jpg' },
          plant_type: plant.type_id,
          light_source: plant.light_id,
        },
        ...
        onSubmit: async (values) => {
            ...
            setIsLoading(true);
            let result = await handleEdit('plant', plant.id, data, 'multipart/form-data');
    ...
    return (
        <form onSubmit={formik.handleSubmit} autoComplete="off">
                ...
                <Button onClick={()=> close('edit-plant')} color="info" sx={{ color: '#fff'}} variant="contained" size="large">
                    Cancel
                </Button>
                </Stack>
            </form>
